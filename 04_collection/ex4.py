...
_, day = max(zip(hours, days)) # max 함수는, tuple 이나 list로 이루어진 iterable 자료형에서 가장 앞에 있는 걸 먼저 확인한다!
print(f"{day}요일")
...
print(f"max 점수: {result[-1]}점, min 점수: {result[0]}")

print(f"max 점수: {max(scores)}점, min 점수: {min(scores)}")
...
print(sum(x * y for _, x, y in stocks))
...
total2 = sum(x * y for _, x, y in zip(*stocks2))
print(f"총액 : {total2:,}원")
