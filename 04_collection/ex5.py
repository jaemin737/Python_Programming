...
d = {"id": 1316, "name": "유재민", "age": 17}
print(d)

# 키로 값 가져오기
print(d["name"])
# print(d["phone"]) -> 키 없음 에러

# 에러가 안나게 하려면?
if "phone" in d:
    print(d["phone"])

print(d.get("phone"))  # None or default로 설정된 값
print(d.get("phone", "전화 없음"))  # default로 설정된 값 -> 전화 없음
...
for i, key in enumerate(d):  # 순서, 키 출력
    print(i, key)

for value in d.values():  # value만 출력
    print(value)

for key, value in d.items():  # key랑 value 둘 다 출력
    print(key, value)
...
print(sorted(d)) # key를 기준으로 key만 정렬
print(sorted(d.items()))  # key와 value까지 정렬
print(dict(sorted(d.items())))  # 다시 딕셔너리로

#  value 기준 정렬하기
def key(x):
    return x[1] # value 값을 꺼내온다

print(dict(sorted(d.items(), key=key))) # value 기준 정렬. key = key 함수
